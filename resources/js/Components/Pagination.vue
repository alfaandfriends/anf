<template>
  <Pagination
    ...
    :itemsPerPage="10"
    :siblingCount="2"
    :default-page="page_data.current_page"
    v-if="page_data.data.length"
  >
    <PaginationList v-slot="{ items }" class="flex justify-center items-center gap-1">
      ...
      <template v-for="(item, index) in items" :key="index">
        <PaginationListItem
          ...
          :value="item.value"
          as-child
          @click="$inertia.get(generatePageUrl(page_data.path + '?page=' + item.value))"
        >
          <Button
            class="w-10 h-10 p-0"
            :variant="item.value === page_data.current_page ? 'default' : 'outline'"
          >
            {{ item.value }}
          </Button>
        </PaginationListItem>

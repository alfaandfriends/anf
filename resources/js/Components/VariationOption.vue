...
<template>
    <div>
        ...
        <div v-else v-for="(option, index) in variationOptions" :key="index" class="mt-1 flex rounded-md shadow-sm">
            ...
            <input @input="emit('update:variation', variationOptions)" type="text" placeholder="stock" :name="'variation_option_stock'+index" :id="'variation_option_stock'+index" class="focus:ring-0 focus:border-indigo-300 flex-1 block sm:text-sm" v-model="option.stock" autocomplete="off"/>

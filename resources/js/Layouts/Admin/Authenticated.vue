      ...
      for (let section_key in this.$page.props.menu) {
        for (let menu_key in this.$page.props.menu[section_key]["menus"]) {
          if (!this.$page.props.menu[section_key]["menus"][menu_key]["menu_route"]) {
            this.menu_setting.push({
              section_key: section_key,
              menu_key: menu_key,
              is_open: false,
            });
          }
          for (let sub_menu in this.$page.props.menu[section_key]["menus"][menu_key][
            "sub_menus"
          ]) {
            const sub_menu_route = this.$page.props.menu[section_key]["menus"][menu_key][
              "sub_menus"
            ][sub_menu]["sub_menu_route"];
            if (route().current() && route().current().startsWith(sub_menu_route)) {
              const menu = this.menu_setting.find(
                (menu_data) =>
                  menu_data.section_key === section_key && menu_data.menu_key === menu_key
              );
              menu.is_open = true;
...
<template>
  <div
    class="grid min-h-screen w-full md:grid-cols-[220px_1fr] lg:grid-cols-[280px_1fr] font-noto"
  >
                  ...
                  <template v-for="(menu, menu_key) in section.menus">
                    <template v-if="menu.menu_route && menu.menu_status == 1">
                      <Link
                        ...
                        :class="
                          route().current() &&
                          route().current().startsWith(menu.menu_route)
                            ? 'text-white tracking-wide bg-slate-900'
                            : ''
                        "
                      >
                        <div class="flex items-center">
                          <span class="mr-3 h-6 w-6"
                            ><img :src="'/images/' + menu.menu_icon"
                          /></span>
                          <span
                            class="whitespace-pre select-none font-semibold tracking-wide"
                            >{{ menu.menu_name }}</span
                          >
                        </div>
                      </Link>
                    ...
                    <template v-else>
                      <Accordion
                        ...
                        v-if="$page.props.can[menu.menu_permission]"
                        :defaultValue="
                          menu_opened.menu_key == menu_key &&
                          menu_opened.section_key == section_key
                            ? `item-${menu_key}`
                            : ''
                        "
                      >
                        <AccordionItem :value="`item-${menu_key}`" :class="'border-0'">
                          <AccordionTrigger
                            class="flex items-center justify-between px-3 py-2 transition cursor-pointer group hover:bg-slate-900 hover:text-slate-200 rounded-lg"
                            :class="
                              menu_opened.menu_key == menu_key &&
                              menu_opened.section_key == section_key
                                ? 'text-white tracking-wide bg-slate-900'
                          ...
                          <AccordionContent>
                            <div class="pl-9 py-0.5">
                              <template
                                v-for="(sub_menu, sub_menu_key) in menu.sub_menus"
                              >
                                <Link
                                  v-if="
                                    $page.props.can[sub_menu.sub_menu_permission] &&
                                    sub_menu.sub_menu_status == 1
                                  "
                                  :href="
                                    sub_menu.sub_menu_route
                                      ? route(sub_menu.sub_menu_route)
                                      : ''
                                  "
                                  ...
                                  :class="
                                    route().current() &&
                                    route().current().startsWith(sub_menu.sub_menu_route)
                                      ? 'text-black tracking-wide bg-slate-300'
                                      : ''
                                  "
                                >
                                  <span
                                    class="whitespace-pre select-none tracking-wide"
                                    >{{ sub_menu.sub_menu_name }}</span
                                  >
                                </Link>
                              </template>
                            </div>
    ...
    <div class="relative flex flex-col">
      <header
        class="flex h-14 items-center gap-4 border-b bg-white shadow px-4 lg:h-[60px] lg:px-6"
      >
        <Sheet>
                          ...
                          >
                            <div class="flex items-center">
                              <span class="mr-3 h-6 w-6"
                                ><img :src="'/images/' + menu.menu_icon"
                              /></span>
                              <span class="select-none font-semibold tracking-wide">{{
                                menu.menu_name
                              }}</span>
                            </div>
                          </Link>
                                  ...
                                  >
                                    <Link
                                      ...
                                      :class="
                                        ...
                                        route()
                                          .current()
                                          .startsWith(sub_menu.sub_menu_route)
                                          ? 'text-black tracking-wide bg-slate-300'
                                          : ''
                                      "
                                    >
                                      <span class="select-none tracking-wide">{{
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem @click="$inertia.get(route('profile'))"
              >Profile</DropdownMenuItem
            >
            <DropdownMenuSeparator />
            <DropdownMenuItem @click="$inertia.post(route('admin.logout'))"
      ...
      <main class="flex flex-1 flex-col gap-4 p-4 lg:p-6 bg-slate-50 w-full">
        ...
        <div
          class="px-6 py-2 bg-blue bg-red-500 space-y-4 lg:flex lg:space-y-0 justify-center text-center lg:justify-between items-center"
          v-if="$page.props.can.impersonate_access || $page.props.can.is_impersonated"
        >
          <span class="text-white text-xs font-semibold whitespace-nowrap"
            >Warning: Please use this feature with caution!</span
          >
          <form
            @submit.prevent="impersonate"
            class="flex flex-col md:flex-row w-full space-x-2 items-center justify-center lg:justify-end"
          >
      ...
      >
        <span class="text-sm"
          >{{ new Date().getFullYear() }} &copy; ALFA and Friends</span
        >
      </div>
    </div>
  </div>

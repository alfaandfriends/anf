                ...
                <div class="mx-auto">
                    <div class="align-middle inline-block min-w-full px-2">
                        ...
                        <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                            ...
                            <template v-if="$page.props.division_managers.data.length">
                                <div class="px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6 bg-gray-300">
                                    <div class="flex-1 flex justify-between sm:hidden">
                                        <a :href="$page.props.division_managers.prev_page_url" v-if="$page.props.division_managers.prev_page_url" class="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50"> Previous </a>
                                        <a :href="$page.props.division_managers.next_page_url" v-if="$page.props.division_managers.next_page_url" class="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50"> Next </a>
                                    </div>
                                    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                                        <div>
                                            <p class="text-sm text-gray-700">
                                                Showing
                                                <span class="font-medium">{{ $page.props.division_managers.from }}</span>
                                                to
                                                <span class="font-medium">{{ $page.props.division_managers.to }}</span>
                                                of
                                                <span class="font-medium">{{ $page.props.division_managers.total }}</span>
                                                results
                                            </p>
                                        </div>
                                        <div>
                                            <nav id="pagination" class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">
                                                <Link v-for="(link, key) in $page.props.division_managers.links"
                                                    :key="key"

...
<template>
    <BreezeAuthenticatedLayout>
        <div class="flex flex-col">
            ...
            <div v-if="$page.props.errors && Object.keys($page.props.errors).length > 0" class="-mt-4 mb-4 bg-red-100 border border-red-200 text-sm text-red-800 rounded-lg p-4 dark:bg-red-800/10 dark:border-red-900 dark:text-red-500" role="alert">
                <div class="font-bold" v-for="(error, index) in $page.props.errors" :key="index">{{ error }}</div>
            </div>
            <!-- Basic Information -->
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 2xl:grid-cols-4 gap-2 mb-6">
                <div class="mb-2 col-span-2 sm:col-span-1">
                    ...
                    <div class="mt-1 flex rounded-md shadow-sm">
                        ...
                        valueProp="value"
                        label="display_name"
                        :placeholder="form.assignee_name"
                        ...
                        :classes="{
                            ...
                            placeholder: 'flex items-center h-full absolute left-0 top-0 pointer-events-none bg-transparent leading-snug pl-3.5 text-slate-700',
                            ...
                            <div class="relative flex items-start w-full">
                                <label :for="'case_status_'+case_status.id" class="cursor-pointer select-none flex space-x-2 w-full text-sm text-gray-600">
                                    <div class="flex items-center h-5">
                                        <input :ref="'case_status_'+case_status.id" :id="'case_status_'+case_status.id" type="checkbox" class="border-gray-200 rounded disabled:opacity-50">
                                    </div>
                        ...
                        <li class="inline-flex items-center gap-x-2.5 py-3 px-4 text-sm font-medium bg-white border text-gray-800 -mt-px first:rounded-t-lg first:mt-0 last:rounded-b-lg sm:-ms-px sm:mt-0 sm:first:rounded-se-none sm:first:rounded-es-lg sm:last:rounded-es-none sm:last:rounded-se-lg" v-for="program, program_index in $page.props.programs" @change="triggerProgram($event, program.id)">
                            <div class="relative flex items-start w-full">
                                <label :for="'program_'+program.id" class="cursor-pointer select-none flex space-x-2 w-full text-sm text-gray-600">
                                    <div class="flex items-center h-5">
                                        <input :ref="'program_'+program.id" :id="'program_'+program.id" type="checkbox" class="border-gray-200 rounded disabled:opacity-50">
                                    </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 2xl:grid-cols-4 gap-2 mb-6">
                ...
                <div v-else>
                    ...
                    <div class="mb-2 flex justify-between items-center shadow-border shadow rounded-lg shadow-gray-300 px-6 py-3">
                        <div class="flex items-center">
                            <div>
                                <p class="text-sm font-medium text-indigo-600 dark:text-white cursor-pointer" @click="downloadQuotation">{{ form.quotation.name ? form.quotation.name : form.quotation }}</p>
                            </div>
                        </div>
                        <div class="items-center">
                            <a class="text-gray-500 hover:text-gray-800 cursor-pointer" @click="form.quotation = '', form.delete_quotation = true">
                ...
                <div v-else>
                    ...
                    <div class="mb-2 flex justify-between items-center shadow-border shadow rounded-lg shadow-gray-300 px-6 py-3">
                        <div class="flex items-center">
                            <div>
                                <p class="text-sm font-medium text-indigo-600 dark:text-white cursor-pointer" @click="downloadContract">{{ form.contract.name ? form.contract.name : form.contract }}</p>
                            </div>
                        </div>
                        <div class="items-center">
                            <a class="text-gray-500 hover:text-gray-800 cursor-pointer" @click="form.contract = '', form.delete_contract = true">
                                <svg class="flex-shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                    <path d="M3 6h18"></path>
                                    <path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"></path>
                                    <path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"></path>
...
export default {
    ...
    methods: {
        ...
        submit(){
            this.$inertia.post(route('crm.sales.pipelines.update'), this.form)
        },

        ...
        <div class="py-4 px-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="flex justify-between pb-4 relative text-gray-400 focus-within:text-gray-600 items-center">
                    <BreezeButton buttonType="gray" :route="route('progress_report.settings.math.units', {'level_id': $page.props.level_id, 'term_book_id': $page.props.term_book_id})">Back</BreezeButton>
                    <BreezeButton @click="create">New Lesson</BreezeButton>
                </div>
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-2">
...
export default {
    ...
    data(){
        return{
            ...
            form:{
                id: '',
                unit_id: this.$page.props.unit_id,
                lesson_name: ''
            },
            errors:{
                lesson_name: false
    ...
    methods: {
        ...
        store(){
            ...
            this.$inertia.post(route('progress_report.settings.math.lessons.store'), this.form, {preserveState: false})
        },
        update(){
            if(!this.form.lesson_name){
                this.errors.lesson_name = true
            ...
            this.$inertia.post(route('progress_report.settings.math.lessons.update'), this.form, {preserveState: false})
        },
        destroy(id){
            this.confirmationRoute = 'progress_report.settings.math.lessons.destroy'
            this.confirmationData = id
            this.isOpen = true
        },
        viewObjectives(lesson_id){
            this.$inertia.get(route('progress_report.settings.math.objectives'), {'level_id': this.$page.props.level_id, 'term_book_id' : this.$page.props.term_book_id, 'unit_id' : this.$page.props.unit_id, 'lesson_id' : lesson_id})
        },
    }
}
</script>
...
export default {
    ...
    data(){
        return{
            form: {
                fee_id: this.fee_info.id,
                fee_type: this.fee_info ? this.fee_info.fee_type : '',
                period: this.fee_info ? this.fee_info.fee_period : '',
                class_per_week: this.fee_info ? this.fee_info.class_per_week : 0,
                class_duration_hours: this.fee_info ? this.fee_info.class_duration_hours : 0,
                class_duration_minutes: this.fee_info ? this.fee_info.class_duration_minutes : 0,
                fee_amount: this.fee_info ? this.fee_info.amount : 0.0,
            }
        }
    },
    methods: {
        submit() {
            this.$inertia.post(route('fees.update'), this.form, { preserveState: true})
        },
    }

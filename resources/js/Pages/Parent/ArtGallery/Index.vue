...
<template>
    ...
    <div class="flex justify-center">
      <div class="flex-1 max-w-4xl">
        <div class="bg-white overflow-hidden shadow rounded-lg border p-6 min-h-[600px]">
          <div
            class="flex flex-col md:flex-row lg:flex-row justify-start md:justify-between lg:justify-between items-start md:items-center"
          >
            ...
            <div
              ...
              v-if="$page.props.current_active_child.student_id"
            >
              <button
                type="button"
                class="flex bg-indigo-600 hover:bg-indigo-700 shadow-sm shadow-slate-400 hover:shadow-slate-600 rounded-lg border-indigo-600 py-2 px-4 items-center justify-center w-full font-semibold text-white"
                @click="showUpload"
              >
                Upload Artwork
              </button>
          ...
          <div
            class="mb-3 flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 justify-between items-end"
          >
            <Multiselect
              ...
              :classes="{
                container:
                  'relative mx-auto w-full flex items-center justify-end box-border rounded-lg border-gray-200 shadow-sm shadow-gray-500 cursor-pointer border border-gray-300 rounded bg-white text-base leading-snug outline-none',
                ...
                tag:
                  'bg-green-500 text-white text-sm font-semibold py-0.5 pl-2 rounded mr-1 mb-1 flex items-center whitespace-nowrap rtl:pl-0 rtl:pr-2 rtl:mr-0 rtl:ml-1',
            ...
            <div
              @click="getArtwork"
              class="w-full md:w-14 md:flex md:items-center rounded-lg bg-indigo-600 hover:bg-indigo-700 shadow-sm shadow-slate-400 hover:shadow-slate-600 text-white p-2.5 cursor-pointer"
            >
              <div class="flex items-center space-x-4">
                <svg
                  v-if="searching.artworks"
                  class="inline w-5 h-5 text-gray-200 animate-spin fill-indigo-600"
                  viewBox="0 0 100 101"
                  fill="none"
          ...
          <div
            v-if="init"
            class="p-4 text-sm text-gray-800 rounded-lg bg-gray-200 text-center"
            role="alert"
          >
            <span class="font-medium">Use filters above to find student artwork.</span>
          </div>
          <div
            v-if="!init && artworks.length < 1"
            class="p-4 text-sm text-gray-800 rounded-lg bg-gray-50 text-center"
            role="alert"
          >
          ...
          <template v-else v-for="(artwork, lesson) in artworks">
            ...
            <div class="flex overflow-x-scroll py-4 scrollbar w-full">
              <div class="flex space-x-4">
                <div
                  ...
                  :key="index"
                  class="relative w-52 h-52 flex flex-col justify-center"
                >
                  <img
                    :src="'/storage/art_gallery/' + image.filename"
                    class="object-fill w-52 h-52 rounded-lg cursor-pointer"
                    alt="Click to view larger image"
                    @click="showImage(image.filename, image.activity)"
                  />
                  <button
                    @click="promptDelete(image.id)"
                    class="absolute bottom-0 w-full bg-red-600 py-2 text-white"
                  >
                    Delete
                  </button>
                </div>
            ...
            <div class="overflow-x-auto">
              <div class="flex space-x-2 py-3"></div>
            </div>
          </template>
          <div
            class="flex items-end justify-end fixed bottom-24 right-4 md:hidden lg:hidden"
            v-if="$page.props.current_active_child.student_id"
          >
            <button
              class="bg-indigo-600 hover:bg-indigo-600 text-white font-semibold p-3 rounded-full shadow-lg"
              @click="showUpload"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
  ...
  <SimpleModal :isOpen="show_upload" @close="this.show_upload = false">
    <form class="p-4">
      ...
      <select
        class="mt-1 focus:ring-0 focus:border-gray-500 flex-1 block w-full rounded-md mb-4 cursor-pointer text-sm md:text-md"
        :class="!errors.level_id ? 'border-gray-300' : 'border-red-300'"
        v-model="form.level_id"
      >
        <option value="">Please Select</option>
        <option :value="level.id" v-for="(level, index) in list.levels" :key="index">
          {{ level.name }}
        </option>
      </select>
      ...
      <select
        class="mt-1 focus:ring-0 focus:border-gray-500 flex-1 block w-full rounded-md mb-4 cursor-pointer text-sm md:text-md"
        :class="!errors.theme_id ? 'border-gray-300' : 'border-red-300'"
        v-model="form.theme_id"
      >
        <option value="">Please Select</option>
        <option :value="theme.id" v-for="(theme, index) in list.themes" :key="theme.id">
          {{ theme.name }}
        </option>
      </select>
      ...
      <select
        class="mt-1 focus:ring-0 focus:border-gray-500 flex-1 block w-full rounded-md mb-4 cursor-pointer text-sm md:text-md"
        :class="!errors.lesson_id ? 'border-gray-300' : 'border-red-300'"
        v-model="form.lesson_id"
      >
        <option value="">Please Select</option>
        <option
          :value="lesson.id"
          v-for="(lesson, index) in list.lessons"
          :key="lesson.id"
        >
          {{ lesson.name }}
        </option>
      ...
      <select
        class="mt-1 focus:ring-0 focus:border-gray-500 flex-1 block w-full rounded-md mb-4 cursor-pointer text-sm md:text-md"
        :class="!errors.activity_id ? 'border-gray-300' : 'border-red-300'"
        v-model="form.activity_id"
      >
        <option value="">Please Select</option>
        <option
          :value="activity.id"
          v-for="(activity, index) in list.activities"
          :key="activity.id"
        >
          {{ activity.name }}
        </option>
      ...
      <div class="flex items-center space-x-2">
        <BreezeButton @click="upload" buttonType="info" class="px-4 py-2">
          <div v-if="uploading" class="flex space-x-2">
            <svg
              aria-hidden="true"
              class="inline w-4 h-4 text-gray-200 animate-spin fill-indigo-600"
              viewBox="0 0 100 101"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
    ...
    <div class="relative text-center p-6">
      ...
      <div class="flex justify-center items-center space-x-4">
        <button
          @click="deleteImage"
          type="button"
          class="py-2 px-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-primary-300 hover:text-gray-900 focus:z-10"
        >
          Confirm
        </button>
        <button
  ...
  watch: {
    "filter.level_id": {
      handler() {
        ...
        axios
          .get(route("parent.art_gallery.get_themes", this.filter.level_id))
          .then((response) => {
  ...
  methods: {
    ...
    getArtwork() {
      if (this.filter.level_id && this.filter.theme_id) {
        ...
        axios
          .get(route("parent.art_gallery.get_artworks", this.filter))
          .then((response) => {
            this.init = false;

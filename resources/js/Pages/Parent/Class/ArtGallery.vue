    ...
    <div class="max-w-xl mx-auto mt-10" @scroll="handleScroll()" ref="scrollContainer">
      ...
      <div class="mb-3 flex flex-col space-y-2 justify-between items-end mt-3">
        <Multiselect
          ...
          :classes="{
            container:
              'relative mx-auto w-full flex items-center justify-end box-border rounded-lg shadow cursor-pointer border border-gray-200 rounded bg-white text-base leading-snug outline-none',
      ...
      <div
        class="bg-white overflow-hidden shadow rounded-lg border p-6"
        v-if="$page.props.artworks.data.length && !searching.artworks"
      >
        <simplebar
          data-simplebar-auto-hide="true"
          class="max-h-96"
          @scroll="handleArtworkScroll"
          ref="artwork_container"
        >
          ...
          <div class="grid grid-cols-2 gap-4 md:grid-cols-3 mt-4" v-if="loading.artworks">
            <div
              class="flex items-center justify-center h-40 mb-4 bg-gray-200 rounded animate-pulse"
            >
              <svg
  ...
  methods: {
    getArtwork() {
      if (this.filter.level_id) {
        ...
        axios
          .get(route("parent.art_gallery.get_artworks"), { params: this.filter })
          .then((response) => {
    ...
    showImage(filename, activity) {
      this.lightbox.images[0].src =
        window.location.origin + "/storage/art_gallery/" + filename;

      this.lightbox.open = !this.lightbox.open;
    },
    handleArtworkScroll() {
      const container = this.$refs.artwork_container.$el.querySelector(
      ...
      if (scrollTop + clientHeight >= scrollHeight - 100) {
        if (this.$page.props.artworks.next_page_url) {
          if (!this.loading.artworks) {
            ...
            axios
              .get(route("parent.student_artworks", this.filter.level_id), {
                params: {

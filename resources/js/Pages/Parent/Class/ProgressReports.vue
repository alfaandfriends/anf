<template>
  ...
  <Authenticated>
    ...
    <div class="max-w-xl mx-auto mt-10">
      <div
        class="flex items-center space-y-3"
        v-if="$page.props.progress_reports.data.length"
      >
      ...
      <div class="mt-3" v-if="$page.props.progress_reports.data.length">
        <div class="bg-white shadow rounded-lg border">
          <simplebar
            ...
            class="max-h-96"
            @scroll="handlePRScroll"
            ref="progress_report_container"
          >
            <ul class="divide-y divide-slate-200 dark:divide-zink-500 px-6 py-4">
              <li
                class="flex items-center gap-3 py-2 first:pt-0 last:pb-0"
                v-for="(progress_report, progress_report_index) in $page.props
                  .progress_reports.data"
              >
                <div class="grow">
                  <h6 class="font-medium">
                    {{ moment(progress_report.month).format("MMMM Y") }}
                  </h6>
                </div>
                <div class="shrink-0">
                  <BreezeButton
                    ...
                    @click="
                      showProgressReport(
                        progress_report.id,
                        progress_report.student_fee_id
                      )
                    "
                    >{{
                      generate.id == progress_report.id && generate.running
                        ? "Generating..."
                        : "View"
                    }}</BreezeButton
                  >
              ...
              <div
                ...
                v-if="loading.progress_reports"
              >
                <div>
                  <div
                    class="h-2.5 bg-gray-300 rounded-full dark:bg-gray-600 w-32 mb-2.5"
                  ></div>
                </div>
...
export default {
  ...
  methods: {
    ...
    handlePRScroll() {
      const container = this.$refs.progress_report_container.$el.querySelector(
      ...
      if (scrollTop + clientHeight >= scrollHeight - 100) {
        if (this.$page.props.progress_reports.next_page_url) {
          if (!this.loading.progress_reports) {
            ...
            axios
              .get(route("parent.student_progress_reports"), {
                params: {

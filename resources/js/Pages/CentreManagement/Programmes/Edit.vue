                            ...
                            <div class="sm:row-span-3">
                                <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-md">
                                    ...
                                    <div class="shadow overflow-hidden border-b border-gray-200 rounded-t-sm rounded-b-none mt-3">
                                        <table class="min-w-full divide-y divide-gray-200 rounded rounded-t">
                                            ...
                                            <tbody>
                                                ...
                                                <tr else class="hover:bg-gray-200" v-for="info, index in form.programme_info">
                                                    ...
                                                    <td class="px-4 py-2 text-center border border-gray-400">{{ $page.props.class_types[info.class_type_id].name}}</td>
                                                    <td class="px-4 py-2 text-center border border-gray-400">
                                                        <span v-if="info.material_fee != null">RM </span>{{ info.material_fee }}
                                                    </td>
                                                    <td class="px-4 py-2 text-center border border-gray-400" v-for="class_type_detail, class_type_detail_index in this.$page.props.class_types_detail">
                                                    ...
                                                    <td class="px-4 py-2 text-center border border-gray-400">
                                                        <BreezeButton buttonType="danger" @click="deleteLevel(index, info.programme_level_id ? info.programme_level_id : info.temp_id)">Delete</BreezeButton>
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
    ...
    data() {
        return {
            ...
            form: {
                ...
                programme_info: this.$page.props.programme_levels ? this.$page.props.programme_levels : [],
                programme_level_to_add: [],
                programme_level_to_delete: [],
    ...
    methods: {
        ...
        addLevel(){
            if(this.level && this.level != 0 && this.material_fee && this.material_fee != 0 && this.class_type && this.fee.length > 0 ){
                ...
                const temp_id   =   this.form.programme_info.length + 1
                ...
                this.form.programme_level_to_add.push({
                    'temp_id':temp_id,
                    'level': this.level,
                    'material_fee': this.material_fee,
                ...
                this.form.programme_level_to_add.sort((firstItem, secondItem)=>firstItem.level - secondItem.level)
                this.clearForm()
            }
        },
        deleteLevel(index, programme_level_id){
            this.form.programme_info.filter((programme_info_data)=>{
                if(programme_info_data.programme_level_id == programme_level_id){
                    this.form.programme_level_to_delete.push(programme_level_id)
                    this.form.programme_info.splice(index, 1)
                }
                else{
                    this.form.programme_level_to_add.filter((programme_level_to_add_data, programme_level_to_add_index)=>{
                        if(programme_level_to_add_data.temp_id == programme_level_id){
                            this.form.programme_level_to_add.splice(programme_level_to_add_index, 1)
                            this.form.programme_info.splice(index, 1)
                        }   
                    })
                }

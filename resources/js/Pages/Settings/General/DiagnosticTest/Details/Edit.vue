                            ...
                            <div class="overflow-hidden w-1/2 p-4">
                                ...
                                <draggable class="w-full" v-model="dtList" v-if="dtList.length > 0" @change="updateSorting">
                                    <div class="mb-1" v-for="(element, index) in dtList" :key="index">
                                        <div class="flex">
                                            <div class="cursor-pointer w-full flex justify-between list-group-item bg-indigo-200 py-2 px-5 text-left" :class="showdtDetails ? 'rounded-tl' : 'rounded-l'" @click="dtEdit(element.id)">
    ...
    methods: {
        ...
        dtEdit(test_id){
            if(this.showdtDetails && this.test_id == test_id){
                ...
                this.$inertia.get(route('settings.diagnostic_test.details.edit'), {'test_id': test_id, 'redirect_url': this.redirect_url}, {preserveScroll: true,})
            }
        },
        dtDelete(test_id){

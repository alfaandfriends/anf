...
<template>
    <div id="progress_report" class="p-5 h-[297mm] w-[210mm]">
        <div class="flex justify-center mb-5 h-[200px]">
            <img src="/images/progress_report/math.png" class="w-full h-[200px]" alt="">
        </div>
            ...
            <table class="w-full">
                <thead>
                    <tr class="border">
                        ...
                        <th class="border-2 border-gray-600 py-2 px-4 text-sm w-7/12">Term / Book / Unit / Lesson</th>
                            ...
                            <td class="border-2 border-gray-600 py-2 px-4 align-top divide-y">
                                <template v-if="JSON.parse(report.report_data).length" v-for="item, item_index in JSON.parse(report.report_data)">
                                    <div class="flex flex-col py-3 items-start">
                                        <span class="underline text-left font-semibold">{{ item.term_book_name }} &nbsp; (Unit {{ item.unit_name }})</span>
                                        <span class="text-left italic">Lesson {{ item.lesson_name }}</span>    

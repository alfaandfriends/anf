        ...
        <Dialog v-model:open="show_progress_report">
            ...
            <template #content>
                <div class="p-1 grid grid-cols-1 gap-4">
                    ...
                    <div class="">
                        <Label>Lesson</Label>
                        <ComboBox :items="$page.props.little_artist_lessons" label-property="name" value-property="id" v-model="search.lesson_id" select-placeholder="Select Lesson" search-placeholder="Search lesson..." @select="getOutcomes(search.lesson_id)"></ComboBox>
                    </div>
                    <div class="">
                        <Label>Outcome</Label>
...
export default {
    ...
    methods: {
        ...
        getOutcomes(lesson_id) {
            ...
            axios.get(route('progress_report.get_little_artist_outcomes', lesson_id))
            .then(response => {
        ...
        addItem() {
            ...
            if(this.search.lesson_id && this.search.outcome_id) {
                const dataExist = this.form.report_data.some(item => item.outcome_id === this.search.outcome_id);
                if(dataExist){
                    alert('Item Exists!')
                }
                else{
                    this.searching = true
                    axios.get(route('progress_report.get_little_artist_objectives', this.search.outcome_id))
                    .then(response => {
                        const new_item  =   {
                            lesson_id: this.search.lesson_id,
                            lesson_name: this.$page.props.little_artist_lessons.find(item => item.id === this.search.lesson_id).name,

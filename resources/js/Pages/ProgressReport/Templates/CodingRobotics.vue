        ...
        <Dialog v-model:open="show_progress_report">
            ...
            <template #content>
                <div class="p-1 grid grid-cols-1 gap-4">
                    ...
                    <div class="">
                        <Label>Lesson</Label>
                        <ComboBox :items="$page.props.coding_lessons" label-property="name" value-property="id" v-model="search.lesson_id" select-placeholder="Select Lesson" search-placeholder="Search lesson..." @select="getTopics(search.lesson_id)"></ComboBox>
                    </div>
                    <div class="">
                        <Label>Topic</Label>
                        <ComboBox :items="options.topics" label-property="name" value-property="id" v-model="search.topic_id" select-placeholder="Select Topic" search-placeholder="Search topic..." @select="getObjectives(search.topic_id)" :loading="loading.topics"></ComboBox>
                    </div>
                    <div class="">
                        <Label>Learning Objective</Label>
                        <ComboBox :items="options.objectives" label-property="name" value-property="id" v-model="search.objective_id" select-placeholder="Select Objective" search-placeholder="Search objective..." @select="addItem()" :loading="loading.objectives"></ComboBox>
                    </div>
                    <div class="mb-3">
                        ...
                        <Collapsible class=" bg-white" v-model="open_objectives" v-else v-for="data, index in form.report_data">
                            <template #trigger>
                                <div class="flex items-center space-x-2">
                                    <Label class="whitespace-pre-line">{{ data.lesson_name }} : {{ data.topic_name }}</Label>
                                    <Label class="text-red-600 cursor-pointer" @click="deleteItem(index)">Delete</Label>
                                </div>
                            </template>
                            <template #content>
                                <div class="p-1">
                                    <Label>{{ data.objective_name }}</Label>
                                </div>
                            </template>
                        </Collapsible>

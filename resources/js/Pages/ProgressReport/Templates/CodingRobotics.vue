      ...
      <template #content>
        <div class="p-1 grid grid-cols-1 gap-4">
          ...
          <div class="">
            <Label>Lesson</Label>
            <ComboBox
              :items="$page.props.coding_lessons"
              label-property="name"
              value-property="id"
          ...
          <div class="">
            ...
            <ComboBox
              ...
              search-placeholder="Search objective..."
              @select="addItem()"
              ...
              <template #trigger>
                <div class="flex items-center space-x-2">
                  <Label class="whitespace-pre-line"
                    >{{ data.lesson_name }} : {{ data.topic_name }}</Label
                  >
...
export default {
  ...
  methods: {
    ...
    getTopics(lesson_id) {
      ...
      axios
        .get(route("progress_report.get_coding_topics", lesson_id))
        .then((response) => {
    ...
    getObjectives(topic_id) {
      ...
      axios
        .get(route("progress_report.get_coding_objectives", topic_id))
        .then((response) => {
    ...
    addItem() {
      ...
      if (this.search.lesson_id && this.search.topic_id && this.search.objective_id) {
        const dataExist = this.form.report_data.some(
          (item) => item.objective_id === this.search.objective_id
        );
        if (dataExist) {
          alert("Item Exists!");
        } else {
          ...
          axios
            .get(
              route(
                "progress_report.get_coding_activities_procedures",
                this.search.objective_id
              )
            )
            .then((response) => {
              const new_item = {
                ...
                lesson_name: this.$page.props.coding_lessons.find(
                ...
                objective_name: this.options.objectives.find(
                  (item) => item.id === this.search.objective_id
                )?.name,
                activities_procedures: [],
              };

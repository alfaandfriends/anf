...
<template>
    ...
    <BreezeAuthenticatedLayout>
        ...
        <div class="py-4 px-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="flex justify-between pb-4 relative text-gray-400 focus-within:text-gray-600 items-center">
                    <BreezeButton buttonType="gray" class="py-2 px-4" :route="route('progress_report.settings.digital_art.levels')">Back</BreezeButton>
                    <BreezeButton class="py-2 px-4" @click="create">New Theme</BreezeButton>
                </div>
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-2">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-300">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-4/12">Theme</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-if="!$page.props.themes.length">
                                ...
                                <tr class="hover:bg-gray-200" v-for="(result, index) in $page.props.themes">
                ...
                <div class="space-y-2" v-if="show_create">
                    <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-md">
                        <div class="mb-5 border-b-indigo-500 border-b border-dashed">
                            <h2 class="font-display font-bold text-indigo-600">New Theme</h2>
                        </div>
                        <div class="grid grid-cols-1 gap-0 sm:gap-4">
                            <div class="mb-4">
                                <label for="theme_name" class="block text-sm text-gray-700 font-bold"> Name <span class="text-red-500">*</span></label>
                                <div class="mt-1 flex rounded-md shadow-sm">
                                    <input type="text" name="theme_name" id="theme_name" class="focus:ring-0 focus:border-indigo-300 flex-1 block w-full rounded-md sm:text-sm" :class="errors.theme_name ? 'border-red-300' : 'border-gray-300'" v-model="form.theme_name" autocomplete="off"/>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <ConfirmationModal 
            ...
            confirmationTitle="Delete Theme"
            confirmationText="Are you sure want to delete this theme? Please be aware that deleting this will also result in the deletion of all associated data. This action cannot be undone."
...
export default {
    ...
    methods: {
        ...
        store(){
            ...
            this.$inertia.post(route('progress_report.settings.digital_art.themes.store'), this.form, {preserveState: false})
        },
        update(){
            if(!this.form.theme_name){
                this.errors.theme_name = true
            ...
            this.$inertia.post(route('progress_report.settings.digital_art.themes.update'), this.form, {preserveState: false})
        },
        destroy(id){
            this.confirmationRoute = 'progress_report.settings.digital_art.themes.destroy'
            this.confirmationData = id
            this.isOpen = true
        },
        viewLessons(theme_id){
            this.$inertia.get(route('progress_report.settings.digital_art.lessons'), {'level_id': this.$page.props.level_id, 'theme_id' : theme_id})
        },
    }
}
</script>
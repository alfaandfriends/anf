            ...
            <Card>
                <template #title>
                    <div class="flex justify-between">
                        <Button @click="$inertia.get(route('progress_report.settings.art_digital.lessons', {'level_id' : $page.props.level_id, 'theme_id' : $page.props.theme_id}))">Back</Button>
...
export default {
    ...
    methods: {
        ...
        store(){
            ...
            this.$inertia.post(route('progress_report.settings.art_digital.activities.store'), this.form, {preserveState: false})
        },
        update(){
            if(!this.form.activity_name){
                this.errors.activity_name = true
            ...
            this.$inertia.post(route('progress_report.settings.art_digital.activities.update'), this.form, {preserveState: false})
        },
        destroy(id){
            this.confirmation.route_name    = 'progress_report.settings.art_digital.activities.destroy'
            this.confirmation.id            = id
            this.confirmation.is_open       = true
        },
        viewOutcomes(activity_id){
            this.$inertia.get(route('progress_report.settings.art_digital.outcomes'), {'level_id': this.$page.props.level_id, 'theme_id': this.$page.props.theme_id, 'lesson_id': this.$page.props.lesson_id, 'activity_id' : activity_id})
        },
    }
}
</script>
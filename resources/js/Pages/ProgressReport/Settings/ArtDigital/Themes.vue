            ...
            <Card>
                <template #title>
                    <div class="flex justify-between">
                        <Button @click="$inertia.get(route('progress_report.settings.art_digital.levels'))">Back</Button>
...
export default {
    ...
    methods: {
        ...
        store(){
            ...
            this.$inertia.post(route('progress_report.settings.art_digital.themes.store'), this.form, {preserveState: false})
        },
        update(){
            if(!this.form.theme_name){
                this.errors.theme_name = true
            ...
            this.$inertia.post(route('progress_report.settings.art_digital.themes.update'), this.form, {preserveState: false})
        },
        destroy(id){
            this.confirmation.route_name    = 'progress_report.settings.art_digital.themes.destroy'
            this.confirmation.id            = id
            this.confirmation.is_open       = true
        },
        viewLessons(theme_id){
            this.$inertia.get(route('progress_report.settings.art_digital.lessons'), {'level_id': this.$page.props.level_id, 'theme_id' : theme_id})
        },
    }
}
</script>
        ...
        <div class="py-4 px-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="flex justify-between pb-4 relative text-gray-400 focus-within:text-gray-600 items-center">
                    <BreezeButton buttonType="gray" :route="route('progress_report.settings.little_bot.levels')">Back</BreezeButton>
                                ...
                                <tr class="hover:bg-gray-200" v-for="(result, index) in $page.props.lessons">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <div class="flex justify-center space-x-2">
                                            ...
                                            <BreezeButton buttonType="blue" @click="viewTopics(result.id)">View Topics</BreezeButton>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
...
export default {
    ...
    methods: {
        ...
        store(){
            ...
            this.$inertia.post(route('progress_report.settings.little_bot.lessons.store'), this.form, {preserveState: false})
        },
        update(){
            if(!this.form.lesson_name){
                this.errors.lesson_name = true
            ...
            this.$inertia.post(route('progress_report.settings.little_bot.lessons.update'), this.form, {preserveState: false})
        },
        destroy(id){
            this.confirmationRoute = 'progress_report.settings.little_bot.lessons.destroy'
            this.confirmationData = id
            this.isOpen = true
        },
        viewTopics(lesson_id){
            this.$inertia.get(route('progress_report.settings.little_bot.topics'), {'level_id': this.$page.props.level_id, 'lesson_id' : lesson_id})
        },
    }
}
</script>
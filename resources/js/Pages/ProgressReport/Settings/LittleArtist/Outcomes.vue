            ...
            <Card>
                <template #title>
                    <div class="flex justify-between">
                        <Button @click="$inertia.get(route('progress_report.settings.little_artist.lessons', {'level_id': $page.props.level_id}))">Back</Button>
...
export default {
    ...
    methods: {
        ...
        store(){
            ...
            this.$inertia.post(route('progress_report.settings.little_artist.outcomes.store'), this.form, {preserveState: false})
        },
        update(){
            if(!this.form.outcome_name){
                this.errors.outcome_name = true
            ...
            this.$inertia.post(route('progress_report.settings.little_artist.outcomes.update'), this.form, {preserveState: false})
        },
        destroy(id){
            this.confirmation.route_name    = 'progress_report.settings.little_artist.outcomes.destroy'
            this.confirmation.id            = id
            this.confirmation.is_open       = true
        },
        viewObjectives(outcome_id){
            this.$inertia.get(route('progress_report.settings.little_artist.objectives'), {'level_id': this.$page.props.level_id, 'lesson_id' : this.$page.props.lesson_id, 'outcome_id' : outcome_id})
        },
    }
}
</script>
            ...
            <Card>
                <template #title>
                    <div class="flex justify-between">
                        <Button @click="$inertia.get(route('progress_report.settings.little_artist.levels'))">Back</Button>
...
export default {
    ...
    methods: {
        ...
        store(){
            ...
            this.$inertia.post(route('progress_report.settings.little_artist.lessons.store'), this.form, {preserveState: false})
        },
        update(){
            if(!this.form.lesson_name){
                this.errors.lesson_name = true
            ...
            this.$inertia.post(route('progress_report.settings.little_artist.lessons.update'), this.form, {preserveState: false})
        },
        destroy(id){
            this.confirmation.route_name    = 'progress_report.settings.little_artist.lessons.destroy'
            this.confirmation.id            = id
            this.confirmation.is_open       = true
        },
        viewOutcomes(lesson_id){
            this.$inertia.get(route('progress_report.settings.little_artist.outcomes'), {'level_id': this.$page.props.level_id, 'lesson_id' : lesson_id})
        },
    }
}
</script>
...
class InvoiceHelper {
    ...
    public static function newFeeInvoice($invoice_data)
    {
        ...
        $totalFee = $invoice_items->sum(function ($item) {
            $programme_fee              =   $item['use_old_fee'] ? $item['old_programme_fee'] : $item['programme_fee'];
            ...
            return $programme_fee + ($registration_fee - $registration_fee_discount) + ($material_fee - $material_fee_discount);
        });
        
        /* Calculate total promo */
        $totalPromo =   0;
        foreach($invoice_items as $fee_key => $fee){
            $programme_fee  =   $fee['use_old_fee'] ? $fee['old_programme_fee'] : $fee['programme_fee'];
            if (isset($fee['promos'])) { // Check if 'promos' key exists
                foreach($fee['promos'] as $promo_key => $promo){
                    if($promo['type_id'] == 1){
                        $promoAmount    =   $programme_fee * $promo['value'] / 100;
    ...
    public static function getStudentFeeInvoices()
    {
        $fee_invoices   =   DB::table('invoices')
                                ...
                                ->where('invoices.student_id', session('current_active_child.student_id'))
                                ...
                                ->orderByDesc('invoices.date_issued')
                                ->paginate(10);
